...
public class NameTest extends BaseClass{
    @ParameterizedTest
    @DisplayName("test with 3 names in request form")
    @ValueSource(strings = { "Гарри Поттер", "Петров Петр Петрович", "Дональд Дак" })

    public void testName(String names) throws InterruptedException {
        ...
        fillInputField(name, names);
        Assertions.assertEquals(name.getAttribute("value"), names);
...
//        element.submit();
    ...
    @Test
    void testExample(){
        Assertions.assertEquals(5, 4, "Not exactly");
    }
}